...
-- WARSZAWASZA · Zapach WARSZAWASZA (PolakPotrafi 2015) · zero natural-person PII
...
-- associated_operator_node = pseudonymous COP node (STUDIO:WAW_DZ3A7 style),
-- aligned with user_reputation_scores.operator_node_id in 011 — NOT user accounts.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS product_flacon_tokens (
  ...
  associated_operator_node VARCHAR(255) DEFAULT NULL,
  ...
  CONSTRAINT chk_product_flacon_operator_node_nonempty
    CHECK (
      associated_operator_node IS NULL
      OR length(trim(associated_operator_node)) > 0
    )
);
...
COMMENT ON COLUMN product_flacon_tokens.associated_operator_node IS
  'Optional COP operator node id (e.g. STUDIO:WAW_DZ3A7). Not end-user identity — no users table in COP.';
...
CREATE INDEX IF NOT EXISTS idx_flacon_operator_node
  ON product_flacon_tokens (associated_operator_node)
  WHERE associated_operator_node IS NOT NULL;
